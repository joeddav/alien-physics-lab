...
/workspace/trl-grpo-venv/bin/python scripts/train_grpo.py \
  --preset real --run-name rl-mw4-lr4e6 \
  --vary-tools --measurement-noise 0.08 \
  --lr 4e-6 --beta 0.005 --num-generations 8 --per-device-batch 2 --grad-accum 16 \
  --gpu-mem-util 0.18 --max-completion-length 4096 --max-steps 200 --no-save-final \
  --wandb --tags "vary-tools,g8,kl0.005,4worlds,lr4e-6,lr2e-6" \
