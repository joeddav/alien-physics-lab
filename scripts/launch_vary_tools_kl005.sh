...
# no-KL run's ~1.0 reward / n~9 before it blew up. Try beta=0.005 (half): weaker anchor ->
...
/workspace/trl-grpo-venv/bin/python scripts/train_grpo.py \
  --preset real --run-name rl-vary-tools-g8-kl005 \
  --vary-tools --measurement-noise 0.08 \
  --lr 2e-6 --beta 0.005 --num-generations 8 --per-device-batch 2 --grad-accum 4 \
  --gpu-mem-util 0.18 --max-completion-length 4096 --max-steps 1000 --no-save-final \
  --wandb --tags "vary-tools,g8,lr2e-6,kl0.005,long-1k,beta-down" \
  --notes "beta-down probe: beta=0.005 (half of the 0.02 run that plateaued ~0.63 / aggregation eroded to 3.5). Same config otherwise. Does the weaker anchor recover reward (~1.0)/aggregation (n~9) while still preventing the beta=0 entropy runaway (which hit ~step 380)? Watch entropy in BOTH directions."
ec=$?
